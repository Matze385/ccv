...
int main(int argc, char** argv)
{
	...
	int dirlen = (base_dir != 0) ? strlen(base_dir) + 1 : 0;
        //read in paths to positive examples in posfiles and bounding box data in bboxes
	while (fscanf(r0, "%s %d %d %d %d", file, &x, &y, &width, &height) != EOF)
	{
		posfiles[size] = (char*)ccmalloc(1024);
		if (base_dir != 0)
		{       
                        //copy name of base dir into each line of posfiles
			strncpy(posfiles[size], base_dir, 1024);
			posfiles[size][dirlen - 1] = '/';
		}
                //copy filename into lines of posfiles->path to file
	...
	char** bgfiles = (char**)ccmalloc(sizeof(char*) * capacity);
        //write paths to negative files in lines of bgfiles
